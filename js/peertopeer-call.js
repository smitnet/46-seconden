...
function setCallListeners(call) {
    call
        .on("localStreamAvailable", function (stream) {
            ...
            addStreamInDiv(stream, 'local-container', 'local-media-' + stream.getId(), {width : "140", height : "auto"}, true);
            stream
        ...
        .on("streamAdded", function (stream) {
            ...
            addStreamInDiv(stream, 'remote-container', 'remote-media-' + stream.getId(), {width : "280", height : "auto"}, false);
        })
...
var ua = new apiRTC.UserAgent({
    uri: 'apzkey:e8e7cc7179b6f799af6ea95d3239d7ff'
});
...
ua.register(registerInformation).then(function (session) {
    ...
    // Display user number
    document.getElementById('my-number').innerHTML = 'Jouw nummer is: ' + connectedSession.id;
...
function addHangupButton(callId) {
    $("#hangupButtons").append('<input id="hangup-' + callId + '" class="btn btn-danger" type="button" value="Ophangen" onclick="hangupCall(' + callId + ')" />');
}
